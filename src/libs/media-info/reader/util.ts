 ...
 * @return {string}
 */
export function getString(view: DataView, length: number | undefined, offset: number = 0, raw?: boolean): string {
 ...
 * @return {string}
 */
export function getStringUtf16(view: DataView, length: number | null, offset: number = 0, bom?: boolean): string {
  ...
  return String.fromCharCode.apply(null, new Uint16Array(str) as unknown as number[]);
}
 ...
 * @return {number}
 */
export function getUint24(view: DataView, offset: number = 0, littleEndian?: boolean): number {
  if (littleEndian) {
